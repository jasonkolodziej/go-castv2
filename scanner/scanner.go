...
// request/reply.
func (s *scanner) GetHwAddr(ofGateway bool) (net.HardwareAddr, error) {
	start := time.Now()
	arpDst := s.dst
	if s.gw == nil && ofGateway {
		return nil, errors.New("gateway address cannot be resolved to obtain HardwareAddress")
	} else if ofGateway {
		arpDst = s.gw
	}
	...
	// is just an example ;)
	log.Printf("Getting MAC Address for IP Destination: %v", arpDst)
...
func (s *scanner) Scan() error {
	...
	hwaddr, err := s.GetHwAddr(DefaultHwAddrParam)
	if err != nil {
		return err
	}
